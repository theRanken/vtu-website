   ...
   $date=date("l j<\s\up>S</\s\up>, F Y @ g:iA");
    ...
    $user = "SELECT * FROM user WHERE username='$name' ";
			...
			$response['message'] = 'something went wrong contact adex developer';
               ...
               $response['title']  = 'opps';
		    $response['status']  = 'error';
			$response['message'] = ' '.$name.', does not have up to dat in his account. Is bal is => '.$bal.'';
               }else{
                   
            $update = mysqli_query($con, "UPDATE  user SET bal='$fund' WHERE username='$name'");
            $updat = mysqli_query($con, "UPDATE  bankpayment SET status='0' WHERE id='$code' ");
            if($updat){
                
            @require "../Mail/phpmailer/PHPMailerAutoload.php";
         require_once "../core/debit.php";
         $mail = new PHPMailer();
        $mail->CharSet = "UTF-8";
         $mail->IsSMTP();
         $mail->SMTPAuth = true;
            ...
            $mail->isHTML(true);
            $mail->Subject = "Debited Transaction";
            $mail->Body = $temp;         
            if(!$mail->send()){
