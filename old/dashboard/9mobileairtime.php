...
$amount = $phone = $amount_err = $phone_err = "";
if(isset($_POST["send"])){
    $amount = htmlspecialchars($_POST["amount"]);
    $phone = htmlspecialchars($_POST["phone"]);
    if(empty($amount)){
       $amount_err = "Please, enter Airtime amount!";
    }else if(!empty($amount) AND $amount < 50){
        $amount_err = "Mininum amount must be N50 !";
    }
    
    if(empty($phone)){
        $phone_err = "Please, enter the phone number that will get the Airtime";
    }
    else if(!empty($phone) AND strlen($phone) < 11 OR strlen($phone) > 11){
        $phone_err = "Phone number must be eleven (11) characters long.";
    }
    else if(empty($amount_err) AND empty($phone_err)){
        ...
        if($myBalance < $amount){
            ...
            //Integrate API
           $endpoint = "https://www.superjaraapi.com/api/topup/";
       ...
       echo $id = $result->id;
       echo $phone_number = $result->mobile_number;
       echo $network_id = $result->network;
       echo $amount_customer_was_charged = $result->paid_amount;
       $amount_customer_received = $result->plan_amount;
        $network_name = $result->plan_network;
        ...
        if($status == "successful"){
           ...
           echo $final_balance = $myBalance - $amount;
           echo $used = $amount_used + $amount;
           //Update DB
			...
			amount_used = '$used'
        		...
        		if(mysqli_query($con, $update)){
        			...
        			$insert9Mobile = "INSERT INTO airtime_transactions(id, airtime_type, network_id, amount_paid_to_customer, phone_recharged, amount_customer_was_charged, network_name, status, date_recharged, customer_id) 
        			VALUES('$id', '$airtime_type', '$network_id', '$amount_customer_received', '$phone_number', '$amount_customer_was_charged', '$network_name', '$status','$purchased_date', '$customer_id')";
        			$inserted = mysqli_query($con, $insert9Mobile);
        			if($inserted){
        		 ...
        		 header("Location: notcool.php?error=Update Failed!");
        			    exit;
        		}
...
<div style="padding:90px 15px 20px 15px" >
    ...
    <h2 class="w3-center">Fill the form below to purchase your 9Mobile Airtime</h2>
...
<input type="text" class="form-control" name="phone" placeholder="Enter the phone number to be recharged..." value="<?php echo $phone; ?>">
</div>
<div class="form-group">
<label>Amount</label>
<span style="display:block; color:red;"><?php echo $amount_err; ?></span>
<input type="text" class="form-control" name="amount" placeholder="Enter Airtime amount..." value="<?php echo $amount; ?>">
</div>
