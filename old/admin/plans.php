...
if(isset( $_SESSION['username'])) {
 ...
 $pc = $_POST['pc'];
  $name = $_POST['name'];
   $price = $_POST['price']; 
   $result=mysqli_query($con,"SELECT * FROM plans WHERE name='$name' AND cid='$type'");
   if(mysqli_num_rows($result)==1){
      $msg="<div class='alert alert-danger'> <span class='btn btn-success'>$name</span> The Name has already been used!
      <br> Please try using a different Name</div>";
      }else{
  $query ="INSERT INTO plans (cid,did,name,price) 
  VALUES ('$type','$pc','$name','$price')";
   if(mysqli_query($con,$query)) {
            
        
               
             echo"<div style='color:green;'><center>".'Success:, <h2 style="color:red";> '.$name.' </h2> plans has been added to your database succesfully'."</center></div>";
     ?>
 <script>
               document.location.href='aplans.php';
           </script>
           <?php
            exit;
      ...
      echo"<div class='alert alert-danger'><center>".'Sorry!! : Categorie was not added, Please try again Later..'."</center></div>";
  }
   ...
   <form method="POST" >
			 ...
			 <div class="form-group">
			      <div class="row justify-content-center" style="margin-left:0px;margin-right:0px;">
                       <h5><strong><i class="fa fa-user"></i> New Plan.</strong></h5>
                     </div>
            ...
            <label for="id_username" class=" requiredField">
                Price<span class="asteriskField"></span>
                   ...
                   <div class="">
                      <div class="input-group-addon">
                    <input type="text" name="price"  class="textinput textInput form-control" required id="id_username" value="<?php if (isset($price)) echo $price; ?>">
                     </div>             
        ...
        <div class="">
                    ...
                    <select name="type" class="select form-control" required id="adex" onchange="active_disactive_user(this.value)">
          
         <?php $sql = "SELECT * FROM categories "; $result = $con->query($sql); if ($result->num_rows > 0) {
              while($rows = $result->fetch_assoc())
              
              $plansValue .="<option data=\"{$rows["id"]}\" value=\"{$rows["type"]}\">{$rows["name"]}=> {$rows["type"]}   </option>"; }?>
    ...
    <?php echo $plansValue?>
        ...
        <input type="hidden" value="" id="pc" name="pc">
    </div>
            ...
            <div class="row justify-content-center"  style="margin-left:0px;margin-right:0px;">
                       ...
                       <div class="form-group">
                         <button name="upload" class="btn btn-primary fa-pull-right" id="submit">Proceed <i class="fa fa-chevron-right" aria-hidden="true"></i></button>
                       </div>
                       </div>
                       </div>
