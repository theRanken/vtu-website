...
$msg="";
  ...
  if (isset($_GET['delete'])) {
    $id =  $_GET['delete'];
    $delete = mysqli_query($con, "DELETE FROM coupon WHERE id=$id");

    if ($delete) {
      $msg = "<div class='alert alert-danger'>
                              Coupon code has been deleted </div>";
?>
      <script>
...
<div class="container">
 <div class="container-fluid bg-secondary text-light" style="padding-bottom:10px;padding-top:5px;">
          <h3 class="mt-4"><i class="fa fa-key" aria-hidden="true"></i>Coupon Codes</h3>
 ...
 <?php
              ...
              if (mysqli_num_rows($result) == 0) {
                $msg = "You Are yet to create a coupon code";
              }
              ?>
              <div class="alas">
                <?= $msg ?>
              ...
              ?>
                <tr>
                 ...
                 <td><?php if($use['status']==0){
                    echo "<span class='btn btn-success' id=str".$use['id'].">Not Being Used</span>";
                  }; if($use['status']==1){
                    echo "<span class='btn btn-danger' id=str".$use['id'].">Used</span>";
                  };?></td>
                  <td></td>
                  <td>
                    <center>
                      <a class="btn btn-primary" href="code.php?id=<?php echo $use['id']; ?>" title="Edit Moderator"><i class="fa fa-edit"></i></a>
                      <a href="?delete=<?php echo $use['id']; ?>" onclick="return confirm('Do you really want to delete this Coupon Code?');" title="Delete" class="btn btn-danger"><i class="fa fa-trash"></i></a>
