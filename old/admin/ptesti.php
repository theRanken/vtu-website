...
$msg="";
  ...
  if (isset($_GET['delete'])) {
    $id =  $_GET['delete'];
    $delete = mysqli_query($con, "DELETE FROM rate WHERE id=$id");

    if ($delete) {
      $msg = "<div class='alert alert-danger'>
                              Testimony has been Deleted </div>";
?>
      <script>
        setTimeout('Redirect()', 5000);

        function Redirect() {
          window.location = "ptesti.php";
        }
      </script>
    <?php
    } else {
  ...
  if (isset($_GET['approve'])) {
    $id =  $_GET['approve'];
    $approve = mysqli_query($con, "UPDATE rate SET status=1  WHERE id=$id");

    if ($approve) {
      $msg = "<div class='alert alert-success'>
                              Testimony has been Approved</div>";
?>
      <script>
        setTimeout('Redirect()', 5000);
  ...
  if (isset($_GET['change'])) {
    $id =  $_GET['change'];
    $change = mysqli_query($con, "UPDATE rate SET status=0  WHERE id=$id");

    if ($approve) {
      $msg = "<div class='alert alert-success'>
                              Testimony has been UnApproved</div>";
?>
      <script>
        setTimeout('Redirect()', 5000);
...
<div class="container">
 ...
 <?php
              ...
              $query = "SELECT * FROM rate WHERE status=0";
              $result = mysqli_query($con, $query);
              $nu = 1;
              if (mysqli_num_rows($result) == 0) {
                $msg = "No Pending Testimoney Yet";
              }
              ?>
              <div class="alas">
                <?= $msg ?>
              ...
              ?>
                <tr>
                 ...
                 <td><?php if($use['status']==0){
                    echo "<span class='btn btn-danger' id=str".$use['id'].">Not Approved</span>";
                  }; if($use['status']==1){
                    echo "<span class='btn btn-success' id=str".$use['id'].">Approved</span>";
                  };?></td>
                  <td></td>
                  <td>
                  <span>

                      <a href="?delete=<?php echo $use['id']; ?>" onclick="return confirm('Do you really want to delete this Testimony?');" title="Delete" class="btn btn-danger"><i class="fa fa-trash"></i></a>
                    </span>
                    <span>
                      <?php
                      if ($status == 1) {
                      ...
                      ?>
                        <a href="?change=<?= $use['id'] ?>" class="text-warning" onclick="return confirm('Do you really want to unapprove this Testimoney?');" title="Unapprove"><i class="fa fa-check" style="font-size:24px"></i>
                        </a> &nbsp;

                      <?php } else {
                      ?>
                        <center>
                        <a class="btn btn-success"  href="?approve=<?= $use['id'] ?>" class="text-success" onclick="return confirm('Do you really want to approve this Testimoney?');" title="Approve"><i class="fa fa-check" style="font-size:24px"></i></a>
                              </center>
                        <a class="btn btn-primary" href="etesti.php?id=<?php echo $use['id']; ?>" title="Edit Moderator"><i class="fa fa-edit"></i></a>
                    ...
                    <span>
                      ...
                      if ($status == 1) {
                      ?>
                        <a href="atesti.php" class="text-primary" onclick="return confirm('Do you really want to leave this page?');" title="view pins"><i class="fa fa-forward" style="font-size:24px"></i>
                        <?php
                      } else {
                        ?>
