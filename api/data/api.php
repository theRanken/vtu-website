...
if (isset($headers["plan"])) 
    $plan = trim($headers["plan"]);
      ...
      */
        ...
        if ($rate == 1) {
            while ($data_plan = mysqli_fetch_array($query_com)) {
                ...
                $data_price = $data_plan[(string) $account_type];
                    ...
                    if (is_numeric($phone)) {
                        ...
                        if ($data_price < $blc) {
                            ...
                            if ($min < $debit) {
                                    ...
                                    if ($adex_restlt == "true") {
                                        ...
                                        if ($doupt) {
                                            ...
                                            if ($result->status == "process") {
                                                $query = mysqli_query(
                                                    $con,
                                                    "INSERT INTO transactions (username,transid,network,service,type,amount,mobile,plans,status,date,oldbal,newbal) VALUES('$username','$transid','$network_name','$service','$data_type','$data_price','$phone','$data_name','1','$date','$blc','$debit') "
                                                );
                                ...
                                $response["status"] = "fail";
                                $response["msg"] =
                                    "You can't withdraw all the money in your account, minimum amount that must be left is ₦" .
                                    number_format($min, 2) .
                                    "";
                            }
                        } else {
